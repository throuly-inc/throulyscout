...
DROP POLICY IF EXISTS "Agents are publicly viewable" ON throulyscout.agents;
DROP POLICY IF EXISTS "Active lenders are publicly viewable" ON throulyscout.lenders;

CREATE POLICY "Block direct client reads of agents"
ON throulyscout.agents FOR SELECT TO anon, authenticated USING (false);

CREATE POLICY "Block direct client reads of lenders"
ON throulyscout.lenders FOR SELECT TO anon, authenticated USING (false);
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = throulyscout AS $$
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = throulyscout AS $$
  SELECT EXISTS (
    SELECT 1 FROM throulyscout.deals
    WHERE id = _deal_id AND (agent_id = _user_id OR client_id = _user_id)
  )
$$;
REVOKE ALL ON FUNCTION throulyscout.user_can_access_deal(uuid, uuid) FROM PUBLIC;
...
USING (
  ...
  AND (
    ...
    OR throulyscout.user_can_access_deal(auth.uid(), NULLIF((storage.foldername(name))[1], '')::uuid)
  )
);
...
ON throulyscout.anonymous_messages FOR SELECT TO authenticated
USING (recipient_id = auth.uid());

-- 4. Explicit deny-by-default reads on lead-capture tables
CREATE POLICY "Block client reads of leads"
ON throulyscout.leads FOR SELECT TO anon, authenticated USING (false);

CREATE POLICY "Block client reads of pricing waitlist"
ON throulyscout.pricing_waitlist FOR SELECT TO anon, authenticated USING (false);

CREATE POLICY "Block client reads of waitlist"
ON throulyscout.waitlist FOR SELECT TO anon, authenticated USING (false);
