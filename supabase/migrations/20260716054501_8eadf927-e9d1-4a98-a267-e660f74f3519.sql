...
-- Revoke broad EXECUTE on SECURITY DEFINER functions from anon/authenticated/throulyscout.
...
DO $$
BEGIN
  IF to_regprocedure('throulyscout.email_queue_dispatch()') IS NOT NULL THEN
  ...
  END IF;
  IF to_regprocedure('throulyscout.email_queue_wake()') IS NOT NULL THEN
...
REVOKE ALL ON FUNCTION throulyscout.consume_usage(text) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION throulyscout.admin_grant_role(uuid, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION throulyscout.admin_revoke_role(uuid, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION throulyscout.admin_set_subscription_tier(uuid, text, text) TO service_role;
GRANT EXECUTE ON FUNCTION throulyscout.consume_usage(text) TO service_role;

-- has_role and get_user_role are referenced by RLS policies evaluated as the
-- querying user, so signed-in users must retain EXECUTE. Revoke only anon/throulyscout.
