...
CREATE POLICY "Users can view own financial profile" ON public.user_financial_profiles
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own financial profile" ON public.user_financial_profiles
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own financial profile" ON public.user_financial_profiles
  FOR UPDATE USING (auth.uid() = user_id);

-- Saved results policies
CREATE POLICY "Users can view own saved results" ON public.saved_results
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own saved results" ON public.saved_results
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own saved results" ON public.saved_results
  FOR DELETE USING (auth.uid() = user_id);

-- Agents are publicly viewable
CREATE POLICY "Agents are publicly viewable" ON public.agents
...
INSERT INTO public.agents (name, email, phone, states, bio, years_experience, specialties) VALUES
