...
-- function public.get_public_sellers(). Direct reads of sellers (which contains PII)
-- remain fully blocked by RLS for anon/authenticated.
DROP POLICY IF EXISTS "Sellers view own data" ON public.sellers;
...
CREATE OR REPLACE FUNCTION public.get_invitation_email_by_code(invite_code text)
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_invitation_email_by_code(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_invitation_email_by_code(text) TO anon, authenticated;
