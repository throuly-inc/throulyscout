...
DROP POLICY IF EXISTS "Admins read audit" ON public.subscription_audit;
CREATE POLICY "Admins read audit" ON public.subscription_audit
...
CREATE OR REPLACE FUNCTION public.admin_set_subscription_tier(
...
BEGIN
  ...
  UPDATE public.profiles SET subscription_tier = _new_tier, updated_at = now()
  WHERE id = _target;
...
END;
$$;
REVOKE ALL ON FUNCTION public.admin_set_subscription_tier(uuid, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_set_subscription_tier(uuid, text, text) TO authenticated, service_role;
...
CREATE POLICY "Task readers must have deal access" ON public.tasks
  FOR SELECT TO authenticated
  USING (
    public.user_can_access_deal(auth.uid(), deal_id)
    AND (assigned_to = auth.uid() OR created_by = auth.uid() OR EXISTS (
      SELECT 1 FROM public.deals d WHERE d.id = tasks.deal_id AND d.agent_id = auth.uid()
    ))
  );

DROP POLICY IF EXISTS "Assigned users can complete tasks" ON public.tasks;
DROP POLICY IF EXISTS "Creators can update tasks" ON public.tasks;
CREATE POLICY "Task updaters must have deal access" ON public.tasks
...
DROP POLICY IF EXISTS "Agent recipients can read incoming connection requests" ON public.connection_requests;
DROP POLICY IF EXISTS "Agent recipients can update connection requests" ON public.connection_requests;
DROP POLICY IF EXISTS "Lender recipients can read incoming connection requests" ON public.connection_requests;
DROP POLICY IF EXISTS "Lender recipients can update connection requests" ON public.connection_requests;
DROP POLICY IF EXISTS "Recipients can update connection requests" ON public.connection_requests;

CREATE POLICY "Agent recipients read" ON public.connection_requests
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.agent_directory a WHERE a.id = to_agent_id AND a.user_id = auth.uid()));
CREATE POLICY "Lender recipients read" ON public.connection_requests
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.lender_directory l WHERE l.id = to_lender_id AND l.user_id = auth.uid()));
CREATE POLICY "Agent recipients update" ON public.connection_requests
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.agent_directory a WHERE a.id = to_agent_id AND a.user_id = auth.uid()));
CREATE POLICY "Lender recipients update" ON public.connection_requests
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.lender_directory l WHERE l.id = to_lender_id AND l.user_id = auth.uid()));
CREATE POLICY "Profile recipients update" ON public.connection_requests
  FOR UPDATE TO authenticated USING (to_user_id = auth.uid());

-- 5) WAITLIST — allow anon insert, block all reads/updates/deletes
DROP POLICY IF EXISTS "Authenticated users can insert waitlist entries" ON public.pricing_waitlist;
CREATE POLICY "Public insert pricing waitlist" ON public.pricing_waitlist
...
CREATE OR REPLACE FUNCTION public.get_agent_directory(_state text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION public.get_lender_directory(_state text DEFAULT NULL::text)
...
CREATE POLICY "Agent logos listing requires auth" ON storage.objects
  FOR SELECT TO authenticated USING (bucket_id = 'agent-logos');
            ...
            WHERE d.file_path = storage.objects.name
              AND (
                ...
                OR (d.visibility IN ('agent_and_client','all_parties')
                    AND EXISTS (SELECT 1 FROM public.deals dl WHERE dl.id = d.deal_id AND dl.client_id = auth.uid()))
              )
          )
        )
      )
...
REVOKE ALL ON FUNCTION public.notify_client_on_stage_change() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.update_updated_at_column() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.update_sellers_updated_at() FROM PUBLIC, anon, authenticated;
...
REVOKE EXECUTE ON FUNCTION public.user_can_access_deal(uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.is_deal_client(uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_agent_client_ids(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_agent_client_ids_for_rls(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_broker_agent_ids(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_broker_agent_ids_for_rls(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_client_agent_ids(uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_client_agent_ids_for_rls(uuid) FROM anon;
