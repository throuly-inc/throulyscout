...
CREATE TABLE IF NOT EXISTS throulyscout.email_send_log (
...
GRANT ALL ON throulyscout.email_send_log TO service_role;

ALTER TABLE throulyscout.email_send_log ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  CREATE POLICY "Service role can read send log"
    ON throulyscout.email_send_log FOR SELECT
...
DO $$ BEGIN
  CREATE POLICY "Service role can insert send log"
    ON throulyscout.email_send_log FOR INSERT
...
DO $$ BEGIN
  CREATE POLICY "Service role can update send log"
    ON throulyscout.email_send_log FOR UPDATE
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_email_send_log_created ON throulyscout.email_send_log(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_email_send_log_recipient ON throulyscout.email_send_log(recipient_email);

-- Backfill: add message_id column to existing tables that predate this migration
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_log ADD COLUMN message_id TEXT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_email_send_log_message ON throulyscout.email_send_log(message_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_email_send_log_message_sent_unique
  ON throulyscout.email_send_log(message_id) WHERE status = 'sent';

-- Backfill: update status CHECK constraint for existing tables that predate new statuses
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_log DROP CONSTRAINT IF EXISTS email_send_log_status_check;
  ALTER TABLE throulyscout.email_send_log ADD CONSTRAINT email_send_log_status_check
    CHECK (status IN ('pending', 'sent', 'suppressed', 'failed', 'bounced', 'complained', 'dlq'));
END $$;

-- Rate-limit state and queue config (single row, tracks Retry-After cooldown + throughput settings)
CREATE TABLE IF NOT EXISTS throulyscout.email_send_state (
...
INSERT INTO throulyscout.email_send_state (id) VALUES (1) ON CONFLICT DO NOTHING;

-- Backfill: add config columns to existing tables that predate this migration
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_state ADD COLUMN batch_size INTEGER NOT NULL DEFAULT 10;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_state ADD COLUMN send_delay_ms INTEGER NOT NULL DEFAULT 200;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_state ADD COLUMN auth_email_ttl_minutes INTEGER NOT NULL DEFAULT 15;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE throulyscout.email_send_state ADD COLUMN transactional_email_ttl_minutes INTEGER NOT NULL DEFAULT 60;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

GRANT ALL ON throulyscout.email_send_state TO service_role;

ALTER TABLE throulyscout.email_send_state ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  CREATE POLICY "Service role can manage send state"
    ON throulyscout.email_send_state FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE OR REPLACE FUNCTION throulyscout.read_email_batch(queue_name TEXT, batch_size INT, vt INT)
...
CREATE OR REPLACE FUNCTION throulyscout.delete_email(queue_name TEXT, message_id BIGINT)
RETURNS BOOLEAN
LANGUAGE plpgsql SECURITY DEFINER
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION throulyscout.move_to_dlq(
...
REVOKE EXECUTE ON FUNCTION throulyscout.enqueue_email(TEXT, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.enqueue_email(TEXT, JSONB) TO service_role;

REVOKE EXECUTE ON FUNCTION throulyscout.read_email_batch(TEXT, INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.read_email_batch(TEXT, INT, INT) TO service_role;

REVOKE EXECUTE ON FUNCTION throulyscout.delete_email(TEXT, BIGINT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.delete_email(TEXT, BIGINT) TO service_role;

REVOKE EXECUTE ON FUNCTION throulyscout.move_to_dlq(TEXT, TEXT, BIGINT, JSONB) FROM PUBLIC;
...
CREATE TABLE IF NOT EXISTS throulyscout.suppressed_emails (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  email TEXT NOT NULL,
...
GRANT ALL ON throulyscout.suppressed_emails TO service_role;

ALTER TABLE throulyscout.suppressed_emails ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  CREATE POLICY "Service role can read suppressed emails"
    ON throulyscout.suppressed_emails FOR SELECT
    USING (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Service role can insert suppressed emails"
    ON throulyscout.suppressed_emails FOR INSERT
    WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_suppressed_emails_email ON throulyscout.suppressed_emails(email);
...
CREATE TABLE IF NOT EXISTS throulyscout.email_unsubscribe_tokens (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
GRANT ALL ON throulyscout.email_unsubscribe_tokens TO service_role;

ALTER TABLE throulyscout.email_unsubscribe_tokens ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  CREATE POLICY "Service role can read tokens"
    ON throulyscout.email_unsubscribe_tokens FOR SELECT
    USING (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Service role can insert tokens"
    ON throulyscout.email_unsubscribe_tokens FOR INSERT
    WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Service role can mark tokens as used"
    ON throulyscout.email_unsubscribe_tokens FOR UPDATE
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_unsubscribe_tokens_token ON throulyscout.email_unsubscribe_tokens(token);
