...
CREATE POLICY "Agents can read linked profiles" ON public.profiles
  FOR SELECT USING (
    EXISTS (
...
-- ============================================
DROP POLICY IF EXISTS "Agents can create own deals" ON public.deals;
DROP POLICY IF EXISTS "Agents can delete own deals" ON public.deals;
...
CREATE POLICY "Agents can select own deals" ON public.deals
...
CREATE POLICY "Clients can read own deals" ON public.deals
  FOR SELECT USING (client_id = auth.uid());

CREATE POLICY "Brokers can read agent deals" ON public.deals
  FOR SELECT USING (
    EXISTS (
...
-- TASKS RLS
-- ============================================
CREATE POLICY "Users can read assigned or created tasks" ON public.tasks
  FOR SELECT USING (assigned_to = auth.uid() OR created_by = auth.uid());

CREATE POLICY "Agents can create tasks" ON public.tasks
  FOR INSERT WITH CHECK (
    public.get_user_role(auth.uid()) IN ('agent', 'broker', 'admin')
  );

CREATE POLICY "Creators can update tasks" ON public.tasks
  FOR UPDATE USING (created_by = auth.uid());

CREATE POLICY "Assigned users can complete tasks" ON public.tasks
  FOR UPDATE USING (assigned_to = auth.uid());

-- ============================================
-- CONTACTS RLS
-- ============================================
CREATE POLICY "Agents can read own contacts" ON public.contacts
  FOR SELECT USING (agent_id = auth.uid());

CREATE POLICY "Agents can create contacts" ON public.contacts
  FOR INSERT WITH CHECK (agent_id = auth.uid());

CREATE POLICY "Agents can update own contacts" ON public.contacts
  FOR UPDATE USING (agent_id = auth.uid());

CREATE POLICY "Agents can delete own contacts" ON public.contacts
  FOR DELETE USING (agent_id = auth.uid());

-- ============================================
-- PROPERTIES RLS
-- ============================================
CREATE POLICY "Authenticated can read active properties" ON public.properties
...
-- ============================================
CREATE POLICY "Deal participants can read timeline" ON public.timeline_entries
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.deals
...
CREATE POLICY "Deal agents can write timeline" ON public.timeline_entries
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.deals
      WHERE deals.id = timeline_entries.deal_id
