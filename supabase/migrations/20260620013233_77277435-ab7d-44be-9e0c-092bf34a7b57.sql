...
DROP POLICY IF EXISTS "public_select_active_properties" ON public.properties;
DROP POLICY IF EXISTS "Public can view active properties" ON public.properties;
DROP POLICY IF EXISTS "anon_select_active_properties" ON public.properties;
...
-- =========================================================
REVOKE SELECT (from_name, from_email, from_phone) ON public.connection_requests FROM authenticated, anon;

CREATE OR REPLACE FUNCTION public.get_connection_request_contact(_request_id uuid)
...
BEGIN
  ...
  IF r.from_user_id = uid
     OR (r.contact_shared = true AND (
           r.to_user_id = uid
           OR EXISTS (SELECT 1 FROM public.agent_directory ad WHERE ad.id = r.to_agent_id AND ad.user_id = uid)
           OR EXISTS (SELECT 1 FROM public.lender_directory ld WHERE ld.id = r.to_lender_id AND ld.user_id = uid)
         )) THEN
...
REVOKE ALL ON FUNCTION public.get_connection_request_contact(uuid) FROM PUBLIC, anon;
...
-- =========================================================
REVOKE SELECT (code) ON public.invitations FROM authenticated, anon;
