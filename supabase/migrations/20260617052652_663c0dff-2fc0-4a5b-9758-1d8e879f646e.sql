...
DROP POLICY IF EXISTS "Agent directory is publicly readable" ON public.agent_directory;
DROP POLICY IF EXISTS "Lender directory is publicly readable" ON public.lender_directory;
...
ON public.agent_directory FOR SELECT TO authenticated
USING (user_id = auth.uid());

CREATE POLICY "Owners can read own lender_directory row"
ON public.lender_directory FOR SELECT TO authenticated
USING (user_id = auth.uid());
