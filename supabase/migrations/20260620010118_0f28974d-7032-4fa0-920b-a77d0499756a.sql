...
ON throulyscout.deal_timeline FOR UPDATE TO authenticated
USING (EXISTS (SELECT 1 FROM throulyscout.deals d WHERE d.id = deal_timeline.deal_id AND d.agent_id = auth.uid()))
WITH CHECK (EXISTS (SELECT 1 FROM throulyscout.deals d WHERE d.id = deal_timeline.deal_id AND d.agent_id = auth.uid()));

CREATE POLICY "Agents can delete timeline entries for their deals"
ON throulyscout.deal_timeline FOR DELETE TO authenticated
USING (EXISTS (SELECT 1 FROM throulyscout.deals d WHERE d.id = deal_timeline.deal_id AND d.agent_id = auth.uid()));
...
ON throulyscout.tasks FOR DELETE TO authenticated
USING (
  created_by = auth.uid()
  OR EXISTS (SELECT 1 FROM throulyscout.deals d WHERE d.id = tasks.deal_id AND d.agent_id = auth.uid())
);

-- invitations: add DELETE policy
CREATE POLICY "Agents delete own invitations"
ON throulyscout.invitations FOR DELETE TO authenticated
USING (auth.uid() = agent_id);

-- connection_requests: add DELETE policy for senders
CREATE POLICY "Senders can withdraw their own connection requests"
ON throulyscout.connection_requests FOR DELETE TO authenticated
