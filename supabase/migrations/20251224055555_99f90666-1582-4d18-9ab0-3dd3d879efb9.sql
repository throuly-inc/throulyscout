...
CREATE TABLE throulyscout.buyer_match_profiles (
...
CREATE TABLE throulyscout.seller_match_listings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE throulyscout.buyer_seller_matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  buyer_profile_id UUID NOT NULL REFERENCES throulyscout.buyer_match_profiles(id) ON DELETE CASCADE,
  seller_listing_id UUID NOT NULL REFERENCES throulyscout.seller_match_listings(id) ON DELETE CASCADE,
...
-- Enable RLS
ALTER TABLE throulyscout.buyer_match_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE throulyscout.seller_match_listings ENABLE ROW LEVEL SECURITY;
ALTER TABLE throulyscout.buyer_seller_matches ENABLE ROW LEVEL SECURITY;

-- RLS Policies for buyer_match_profiles
CREATE POLICY "Users can view own buyer profiles" ON throulyscout.buyer_match_profiles
  FOR SELECT USING (auth.uid() = user_id);
  
CREATE POLICY "Users can create own buyer profiles" ON throulyscout.buyer_match_profiles
  FOR INSERT WITH CHECK (auth.uid() = user_id);
  
CREATE POLICY "Users can update own buyer profiles" ON throulyscout.buyer_match_profiles
  FOR UPDATE USING (auth.uid() = user_id);
  
CREATE POLICY "Users can delete own buyer profiles" ON throulyscout.buyer_match_profiles
  FOR DELETE USING (auth.uid() = user_id);

-- RLS Policies for seller_match_listings
CREATE POLICY "Active listings are publicly viewable" ON throulyscout.seller_match_listings
  FOR SELECT USING (is_active = true);
  
CREATE POLICY "Users can create own seller listings" ON throulyscout.seller_match_listings
  FOR INSERT WITH CHECK (auth.uid() = user_id);
  
CREATE POLICY "Users can update own seller listings" ON throulyscout.seller_match_listings
  FOR UPDATE USING (auth.uid() = user_id);
  
CREATE POLICY "Users can delete own seller listings" ON throulyscout.seller_match_listings
  FOR DELETE USING (auth.uid() = user_id);

-- RLS Policies for buyer_seller_matches
CREATE POLICY "Buyers can view own matches" ON throulyscout.buyer_seller_matches
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM throulyscout.buyer_match_profiles
      WHERE id = buyer_profile_id AND user_id = auth.uid()
    )
  );

CREATE POLICY "Sellers can view matches for their listings" ON throulyscout.buyer_seller_matches
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM throulyscout.seller_match_listings
      WHERE id = seller_listing_id AND user_id = auth.uid()
    )
  );

CREATE POLICY "Buyers can update own match status" ON throulyscout.buyer_seller_matches
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM throulyscout.buyer_match_profiles
      WHERE id = buyer_profile_id AND user_id = auth.uid()
...
CREATE TRIGGER update_buyer_match_profiles_updated_at
  BEFORE UPDATE ON throulyscout.buyer_match_profiles
  FOR EACH ROW
  EXECUTE FUNCTION throulyscout.update_updated_at_column();

CREATE TRIGGER update_seller_match_listings_updated_at
  BEFORE UPDATE ON throulyscout.seller_match_listings
  FOR EACH ROW
  EXECUTE FUNCTION throulyscout.update_updated_at_column();

CREATE TRIGGER update_buyer_seller_matches_updated_at
  BEFORE UPDATE ON throulyscout.buyer_seller_matches
  FOR EACH ROW
  EXECUTE FUNCTION throulyscout.update_updated_at_column();

-- Index for faster matching queries
CREATE INDEX idx_seller_listings_state ON throulyscout.seller_match_listings(state);
CREATE INDEX idx_seller_listings_price ON throulyscout.seller_match_listings(listing_price);
CREATE INDEX idx_seller_listings_active ON throulyscout.seller_match_listings(is_active);
CREATE INDEX idx_buyer_profiles_active ON throulyscout.buyer_match_profiles(is_active);