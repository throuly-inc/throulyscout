...
BEGIN
  ...
  IF uid IS NOT NULL THEN
    DELETE FROM throulyscout.user_roles WHERE user_id = uid AND role = 'admin'::throulyscout.app_role;
    INSERT INTO throulyscout.user_roles (user_id, role)
      VALUES (uid, 'user'::throulyscout.app_role)
      ON CONFLICT (user_id, role) DO NOTHING;
    PERFORM set_config('app.privileged_update', 'on', true);
    UPDATE throulyscout.profiles SET subscription_tier = 'free', updated_at = now() WHERE id = uid;
    PERFORM set_config('app.privileged_update', 'off', true);
  END IF;
END $$;

CREATE TABLE IF NOT EXISTS throulyscout.admin_role_audit (
...
GRANT SELECT ON throulyscout.admin_role_audit TO authenticated;
GRANT ALL ON throulyscout.admin_role_audit TO service_role;
ALTER TABLE throulyscout.admin_role_audit ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Admins can view role audit" ON throulyscout.admin_role_audit;
CREATE POLICY "Admins can view role audit" ON throulyscout.admin_role_audit
...
CREATE OR REPLACE FUNCTION throulyscout.admin_grant_role(_target uuid, _role text, _reason text DEFAULT NULL)
...
BEGIN
  ...
  END IF;
  INSERT INTO throulyscout.user_roles (user_id, role)
  VALUES (_target, _role::throulyscout.app_role)
  ON CONFLICT (user_id, role) DO NOTHING;
  INSERT INTO throulyscout.admin_role_audit(actor_id, target_user_id, action, role, reason)
  VALUES (auth.uid(), _target, 'grant', _role, _reason);
  RETURN jsonb_build_object('ok', true);
END $fn$;
REVOKE ALL ON FUNCTION throulyscout.admin_grant_role(uuid, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION throulyscout.admin_grant_role(uuid, text, text) TO authenticated;

CREATE OR REPLACE FUNCTION throulyscout.admin_revoke_role(_target uuid, _role text, _reason text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
BEGIN
  IF NOT throulyscout.has_role(auth.uid(), 'admin'::throulyscout.app_role) THEN
    RAISE EXCEPTION 'admin role required' USING ERRCODE = '42501';
  ...
  END IF;
  DELETE FROM throulyscout.user_roles WHERE user_id = _target AND role = _role::throulyscout.app_role;
  INSERT INTO throulyscout.admin_role_audit(actor_id, target_user_id, action, role, reason)
  VALUES (auth.uid(), _target, 'revoke', _role, _reason);
  RETURN jsonb_build_object('ok', true);
END $fn$;
REVOKE ALL ON FUNCTION throulyscout.admin_revoke_role(uuid, text, text) FROM PUBLIC, anon, authenticated;
...
REVOKE ALL ON FUNCTION throulyscout.admin_set_subscription_tier(uuid, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION throulyscout.admin_set_subscription_tier(uuid, text, text) TO service_role;

-- =========================================================
-- 2. INVITATIONS: hashed codes, auth required, generic responses
-- =========================================================
ALTER TABLE throulyscout.invitations
  ADD COLUMN IF NOT EXISTS code_hash text,
  ADD COLUMN IF NOT EXISTS attempts integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS used_at timestamptz;
CREATE INDEX IF NOT EXISTS idx_invitations_code_hash ON throulyscout.invitations(code_hash);

DROP FUNCTION IF EXISTS throulyscout.get_invitation_email_by_code(text);
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    UPDATE throulyscout.invitations SET attempts = attempts + 1
  ...
  IF lower(inv.email) <> lower((SELECT email FROM auth.users WHERE id = uid)) THEN
    UPDATE throulyscout.invitations SET attempts = attempts + 1 WHERE id = inv.id;
    RETURN generic_err;
  END IF;

  UPDATE throulyscout.invitations
  ...
  UPDATE throulyscout.contacts SET linked_user_id = uid WHERE id = inv.contact_id;
  ...
  SELECT full_name INTO agent_name FROM throulyscout.profiles WHERE id = inv.agent_id;

  RETURN jsonb_build_object('success', true, 'agent_name', COALESCE(agent_name, 'Your Agent'), 'agent_id', inv.agent_id);
END $fn$;

REVOKE ALL ON FUNCTION throulyscout.accept_invitation(text) FROM PUBLIC, anon;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT EXISTS (SELECT 1 FROM throulyscout.user_roles WHERE user_id = _user_id AND role = _role)
$$;

CREATE OR REPLACE FUNCTION throulyscout.user_can_access_deal(_user_id uuid, _deal_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT EXISTS (SELECT 1 FROM throulyscout.deals WHERE id = _deal_id AND (agent_id = _user_id OR client_id = _user_id))
$$;
...
BEGIN
  ...
  END IF;
  SELECT COALESCE(subscription_tier, 'free') INTO tier FROM throulyscout.profiles WHERE id = uid;
  ...
  END IF;
  UPDATE throulyscout.usage_counters SET count = count + 1, updated_at = now()
...
RETURNS SETOF uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT DISTINCT agent_id FROM throulyscout.deals WHERE client_id = _client_id
$$;
...
RETURNS SETOF uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT DISTINCT client_id FROM throulyscout.deals WHERE agent_id = _agent_id AND client_id IS NOT NULL
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT EXISTS (SELECT 1 FROM throulyscout.deals WHERE id = _deal_id AND client_id = _user_id)
$$;

CREATE OR REPLACE FUNCTION throulyscout.get_public_agents(_state text DEFAULT NULL)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  ...
  FROM throulyscout.agents a
  WHERE a.is_active = true AND (_state IS NULL OR _state = ANY(a.states))
  LIMIT 50;
$$;

CREATE OR REPLACE FUNCTION throulyscout.get_public_lenders(_state text DEFAULT NULL)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  ...
  FROM throulyscout.sellers WHERE is_active = true;
$$;
...
BEGIN
  ...
  SELECT onboarding_complete INTO current_onboarding FROM throulyscout.profiles WHERE id = auth.uid();
  ...
  UPDATE throulyscout.profiles SET role = new_role, onboarding_complete = true, updated_at = now()
  WHERE id = auth.uid();
END $fn$;
...
BEGIN
  ...
  INSERT INTO throulyscout.user_roles (user_id, role) VALUES (auth.uid(), 'user'::throulyscout.app_role)
  ON CONFLICT (user_id, role) DO NOTHING;
END $fn$;

-- admin_set_subscription_tier with search_path=''
CREATE OR REPLACE FUNCTION throulyscout.admin_set_subscription_tier(_target uuid, _new_tier text, _reason text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
...
BEGIN
  ...
  PERFORM set_config('app.privileged_update', 'on', true);
  UPDATE throulyscout.profiles SET subscription_tier = _new_tier, updated_at = now() WHERE id = _target;
  PERFORM set_config('app.privileged_update', 'off', true);
...
-- =========================================================
REVOKE ALL ON FUNCTION throulyscout.get_public_sellers() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_public_sellers() TO authenticated;

REVOKE ALL ON FUNCTION throulyscout.get_public_agents(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.get_public_agents(text) TO anon, authenticated;
REVOKE ALL ON FUNCTION throulyscout.get_public_lenders(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.get_public_lenders(text) TO anon, authenticated;
REVOKE ALL ON FUNCTION throulyscout.get_agent_directory(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION throulyscout.get_agent_directory(text) TO anon, authenticated;
REVOKE ALL ON FUNCTION throulyscout.get_lender_directory(text) FROM PUBLIC;
...
REVOKE ALL ON FUNCTION throulyscout.has_role(uuid, throulyscout.app_role) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.has_role(uuid, throulyscout.app_role) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.user_can_access_deal(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.user_can_access_deal(uuid, uuid) TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION throulyscout.is_deal_client(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.is_deal_client(uuid, uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_user_role(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_user_role(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_client_agent_ids(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_client_agent_ids(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_client_agent_ids_for_rls(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_client_agent_ids_for_rls(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_agent_client_ids(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_agent_client_ids(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_agent_client_ids_for_rls(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_agent_client_ids_for_rls(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_broker_agent_ids(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_broker_agent_ids(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION throulyscout.get_broker_agent_ids_for_rls(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.get_broker_agent_ids_for_rls(uuid) TO authenticated, service_role;

REVOKE ALL ON FUNCTION throulyscout.set_user_role(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION throulyscout.set_user_role(text) TO authenticated;
REVOKE ALL ON FUNCTION throulyscout.complete_onboarding(text) FROM PUBLIC, anon;
...
-- =========================================================
CREATE OR REPLACE FUNCTION throulyscout.connection_requests_enforce_sender()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
DECLARE
  p record;
BEGIN
  ...
  SELECT full_name, email, phone INTO p FROM throulyscout.profiles WHERE id = auth.uid();
...
DROP TRIGGER IF EXISTS trg_connection_requests_enforce_sender ON throulyscout.connection_requests;
CREATE TRIGGER trg_connection_requests_enforce_sender
  BEFORE INSERT OR UPDATE ON throulyscout.connection_requests
  FOR EACH ROW EXECUTE FUNCTION throulyscout.connection_requests_enforce_sender();
...
DROP POLICY IF EXISTS "anon_insert_anonymous_messages" ON throulyscout.anonymous_messages;
DROP POLICY IF EXISTS "Authenticated users can send anonymous messages" ON throulyscout.anonymous_messages;
...
CREATE OR REPLACE FUNCTION throulyscout.anonymous_messages_enforce_sender()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $fn$
BEGIN
  ...
  RETURN NEW;
END $fn$;
DROP TRIGGER IF EXISTS trg_anonymous_messages_enforce_sender ON throulyscout.anonymous_messages;
CREATE TRIGGER trg_anonymous_messages_enforce_sender
  BEFORE INSERT ON throulyscout.anonymous_messages
  FOR EACH ROW EXECUTE FUNCTION throulyscout.anonymous_messages_enforce_sender();

-- =========================================================
-- 7. WAITLISTS: revoke direct INSERT; unique normalized email
-- =========================================================
ALTER TABLE throulyscout.waitlist ADD COLUMN IF NOT EXISTS normalized_email text
  GENERATED ALWAYS AS (lower(trim(email))) STORED;
ALTER TABLE throulyscout.pricing_waitlist ADD COLUMN IF NOT EXISTS normalized_email text
  GENERATED ALWAYS AS (lower(trim(email))) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS idx_waitlist_normalized_email ON throulyscout.waitlist(normalized_email);
CREATE UNIQUE INDEX IF NOT EXISTS idx_pricing_waitlist_normalized_tier
  ON throulyscout.pricing_waitlist(normalized_email, desired_tier);

DROP POLICY IF EXISTS "Anyone can join waitlist" ON throulyscout.waitlist;
DROP POLICY IF EXISTS "anon insert waitlist" ON throulyscout.waitlist;
DROP POLICY IF EXISTS "public_insert_waitlist" ON throulyscout.waitlist;
DROP POLICY IF EXISTS "Anyone can join pricing waitlist" ON throulyscout.pricing_waitlist;
DROP POLICY IF EXISTS "anon insert pricing_waitlist" ON throulyscout.pricing_waitlist;
DROP POLICY IF EXISTS "public_insert_pricing_waitlist" ON throulyscout.pricing_waitlist;

REVOKE INSERT ON throulyscout.waitlist FROM anon, authenticated;
REVOKE INSERT ON throulyscout.pricing_waitlist FROM anon, authenticated;
GRANT INSERT ON throulyscout.waitlist TO service_role;
GRANT INSERT ON throulyscout.pricing_waitlist TO service_role;
...
FOR ALL TO authenticated
USING (bucket_id = 'throulyscout-agent-logos' AND (storage.foldername(name))[1] = auth.uid()::text)
WITH CHECK (
